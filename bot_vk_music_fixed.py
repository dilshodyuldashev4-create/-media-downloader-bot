...
if not FFMPEG_AVAILABLE:
    log.warning("ffmpeg не найден в PATH — MP3, обложки и совместимость видео могут не работать.")
if not FFPROBE_AVAILABLE:
    log.warning("ffprobe не найден в PATH — кодеки видео нельзя будет проверить заранее.")
...
def search_youtube(query: str, limit: int = SEARCH_LIMIT) -> list[dict]:
    """
    Надёжный поиск музыки по названию/исполнителю.
    AudD здесь не используется: он нужен только для распознавания аудио.
    """
    query = re.sub(r"\\s+", " ", (query or "").strip())
    if not query:
        return []

    # Для поиска не передаём player_client=android/web:
    # на Railway это иногда даёт пустую выдачу без явной ошибки.
    ...
    with yt_dlp.YoutubeDL(opts) as ydl:
        for search_query in search_queries:
            try:
                ...
                log.exception("Ошибка YouTube-поиска по запросу: %s", search_query)
                continue
...
def _run_command(command: list[str], timeout: int) -> subprocess.CompletedProcess:
    """Запускает ffmpeg/ffprobe и возвращает понятную ошибку при сбое."""
    try:
        ...
        raise RuntimeError(f"Команда превысила лимит времени ({timeout} сек.)") from exc
    except OSError as exc:
        raise RuntimeError(f"Не удалось запустить {command[0]}: {exc}") from exc
...
def probe_media(path: Path) -> dict:
    """Возвращает кодеки и параметры контейнера через ffprobe."""
    if not FFPROBE_AVAILABLE:
        return {}
...
def video_is_telegram_compatible(path: Path) -> bool:
    """Telegram/iPhone стабильно воспроизводит MP4 с H.264 + AAC."""
    if path.suffix.lower() != ".mp4":
        return False
    data = probe_media(path)
    if not data:
        # Если ffprobe отсутствует, считаем MP4 приемлемым и не ломаем загрузку.
        return True
    ...
    pixel_ok = not pixel_format or pixel_format in {"yuv420p", "yuvj420p"}
    return video_ok and audio_ok and pixel_ok


def normalize_video_for_telegram(source: Path, workdir: Path, quality: int) -> Path:
    """Перекодирует несовместимый ролик в H.264/AAC MP4 для Telegram и iPhone."""
    if video_is_telegram_compatible(source):
        return source
    if not FFMPEG_AVAILABLE:
        log.warning("Видео несовместимо, но ffmpeg отсутствует; отправляю исходный файл")
        return source

    target = workdir / f"telegram_{quality}p.mp4"
    max_height = max(240, int(quality))
    command = [
        "ffmpeg", "-y", "-loglevel", "error",
        "-i", str(source),
        "-map", "0:v:0", "-map", "0:a:0?",
        "-vf", f"scale=-2:'min({max_height},ih)',format=yuv420p",
    ...
    if not target.exists() or target.stat().st_size == 0:
        raise RuntimeError("ffmpeg не создал совместимый MP4-файл")
    return target
...
def try_download(
    ...
    if kind == "video":
        # Сначала просим H.264/AAC. Последние варианты позволяют скачать ролик,
        # даже когда Instagram/TikTok не предлагают заранее совместимый поток;
        # после загрузки такой файл будет автоматически перекодирован.
        opts.update({
            "format": (
                f"bestvideo[vcodec^=avc1][height<={quality}]+bestaudio[acodec^=mp4a]/"
                f"bestvideo[vcodec^=h264][height<={quality}]+bestaudio[acodec^=aac]/"
                f"best[ext=mp4][vcodec^=avc1][height<={quality}]/"
    ...
    if kind == "video":
        path = normalize_video_for_telegram(path, workdir, quality)
    return path, info or {}
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not tracks:
        await update.message.reply_text("Поиск временно не дал результатов. Напиши исполнителя и название песни, например: Eminem - Mockingbird.")
        return
